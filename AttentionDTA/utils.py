...
def split(config,X_drug,X_target,y,ddi,skipped):
    ...
    X_drug['index'] = X_drug.index
    X_drug['Drug_ID'] = X_drug.index
    X_target['index'] = X_target.index
    X_target['Prot_ID'] = X_target.index
    ...
    #train_ind, test_ind = data_leak(train_ind,test_ind,leak=4)
    ...
    print(f'Util Number of samples in training: {len(train_ind)}')
    print(f'Util Number of samples in validation: {len(val_ind)}')
    print(f'Util Number of samples in test: {len(test_ind)}')
    ...
    for drug in test_ind.Drug_ID.unique():
        for _ in range(leak):
            ...
            if len(ind) <= 1:
                ...
                #choose a random sample from train_ind and add it to test_ind
                #random_row = np.random.choice(train_ind.index,1)
                #test_ind = test_ind.append(train_ind.loc[random_row])
                #train_ind = train_ind.drop(random_row)
...
def pre_split(X_drug,X_target,y,ddi,skipped):
    ...
    #train_ind, test_ind = data_leak(train_ind,test_ind,leak=5)
    #print(np.unique(test_ind.label,return_counts=True))
