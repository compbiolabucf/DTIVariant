...
class Net(pl.LightningModule):
    def __init__(self, cfg,dataset,network,optimizer,criterion,GAT_params):
        ...
        self.cross_attention_layer1 = nn.MultiheadAttention(embed_dim = 1, num_heads = 1,batch_first=True)
        self.cross_attention_layer2 = nn.MultiheadAttention(embed_dim = 1, num_heads = 1,batch_first=True)
        
        self.joint_attention_layer1 = nn.Linear(768,768)
        self.joint_attention_layer2 = nn.Linear(1280,1280)
        self.x1_V = nn.Linear(768,768)
        self.x2_V = nn.Linear(1280,1280)
        self.relu = nn.ReLU()
    ...
    def forward(self, x1,x2,x1_org,x2_org,x1_network,x2_network,inv_drug,inv_target):
        ...
        if self.cfg['module']['GAT_params']['concat']['concat']:
            ...
            x1 = x1+(x1_org*alpha)
            x2 = x2+(x2_org*alpha)
        if False: # Cross-Attention
            x1 = x1.unsqueeze(2)
            x2 = x2.unsqueeze(2)
            x11, x1atn = self.cross_attention_layer1(x1,x2,x2)
            x22, x2atn = self.cross_attention_layer1(x2,x1,x1)
            x1 = x11.squeeze(2)
            x2 = x22.squeeze(2)
        elif False: # Joint-Attention
            x1 = x1.unsqueeze(1)
            x2 = x2.unsqueeze(1)
            x1_joint = self.joint_attention_layer1(self.relu(x1))
            x2_joint = self.joint_attention_layer2(self.relu(x2))
            inter_x1_x2 = self.sigmoid(torch.einsum('bki,bkj->bij', x1_joint, x2_joint))
            inter_x1_x2_sum = torch.einsum('bij->b', inter_x1_x2)
            inter_x1_x2 = torch.einsum('bij,b->bij', inter_x1_x2, 1/inter_x1_x2_sum)
            att = inter_x1_x2
            
            b1 = self.x1_V(x1)
            b2 = self.x2_V(x2)
            x12_embedding = self.tanh(torch.einsum('bki,bkj->bij', b1, b2))
            x12_embedding = torch.einsum('bij,bij->bij', x12_embedding, att)
            x1 = torch.einsum('bjk,bji->bk', x12_embedding, x1)
            x2 = torch.einsum('bjk,bki->bj', x12_embedding, x2)
    ...
    def training_step(self, batch, batch_idx):
        x1,x2,y, drugs, targets, smile, seq, pdb_id = batch
        x1_org , x2_org = x1, x2
    ...
    def test_step(self, batch, batch_idx):
        ...
        x1,x2,x1_network,x2_network, inv_drug, inv_target = self.common_preprocess(x1,x2,drugs,targets)
        loss, scores = self.common_step(x1,x2,x1_org,x2_org,x1_network,x2_network, y, inv_drug, inv_target, batch_idx)
        self.test_step_outputs.append({"loss":loss, "scores":scores, "y":y,"drug_id":drugs,"prot_id":targets, "pdb":pdb_id, "smiles":smile, "seqs":seq})
        return loss
    ...
    def common_preprocess(self, x1,x2,drugs,targets):
        ...
        for index, value in enumerate(drugs):
            if value not in mapping.keys():
                mapping[value] = index
        drugs = torch.tensor([mapping[value] for value in drugs])
        
        mapping = {}
        for index, value in enumerate(targets):
            if value not in mapping.keys():
                mapping[value] = index
        targets = torch.tensor([mapping[value] for value in targets])
    ...
    def for_test_epoch(self, outputs):          
        ...
        pdbs = [y for x in outputs for y in x["pdb"]]
        smiles = [y for x in outputs for y in x["smiles"]]
        seqs = [y for x in outputs for y in x["seqs"]]
        drug_ids = [y for x in outputs for y in x["drug_id"]]
        prot_ids = [y for x in outputs for y in x["prot_id"]]
        test_log = {"loss":avg_loss,"scores":scores,"y":y,"auc":auc,"auprc":auprc,"conf_mat":bcm,"f1":f1,"pdbs":pdbs,"smiles":smiles,"seqs":seqs,"drug_ids":drug_ids, "prot_ids":prot_ids}
        # variant = "variant" if self.cfg['variant_pred']['pred'] else "default"
        tr = self.cfg['variant_pred']['train'] 
        tst = self.cfg['variant_pred']['test']
        dataname = self.cfg['logger']['name'][:4]
        import pickle
        with open(f"3d_analysis/{tr}.{tst}.{dataname}.pickle", "wb") as f:
