...
def get_data(config):
    ...
    if False:
        ...
        data = data.sort_index()
        with open("datasets/bindingDB/BindingDB_All_202406_imputed_processed_new2.pkl","wb") as f:
            pickle.dump(data,f)
        print("Svaed data")
    else:
        with open("datasets/bindingDB/BindingDB_All_202406_imputed_processed_new2.pkl","rb") as f:
            data = pickle.load(f)
    '''
...
def process_data(config):
    ...
    with open("process_data/Protein_Drug_to_complex_map.pickle", "rb") as f:
        prot_drug_com = pickle.load(f)
    def get_complex_list(row,X_drug, X_target):
        did = row["Drug_ID"]
        pid = row["Prot_ID"]
        drug = X_drug.loc[did]['SMILES']
        prot = X_target.loc[pid]['SEQ']
        try:
            return prot_drug_com[prot][drug]
        except:
            return  []
    DTI['Complex'] = DTI.apply(lambda x: get_complex_list(x,X_drug,X_target),axis=1)
